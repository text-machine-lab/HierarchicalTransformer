...
sys.path.append('..')
from translation_dataset import RAFTranslationDataset, paired_collate_fn  # noqa: E402
...
class TestRAFTranslationDataset(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_creation(self):
        _ = RAFTranslationDataset('test_src.txt', 'test_tgt.txt',
                                  mock_tokenizer, mock_tokenizer)

    def test_creation_warmup(self):
        _ = RAFTranslationDataset('test_src.txt', 'test_tgt.txt',
                                  mock_tokenizer, mock_tokenizer, warm_up=True)

    def test_getitem(self):
        ds = RAFTranslationDataset('test_src.txt', 'test_tgt.txt',
                                   mock_tokenizer, mock_tokenizer)
        res = ds[0]
